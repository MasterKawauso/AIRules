...
コード変更のセルフレビュー、コードレビュー、コード変更の完了報告前だけに適用する。文書編集・調査・回答・説明・設計・企画等の非コード作業には自動適用しない。ユーザーが非コード成果物のレビューを明示した場合は、その依頼範囲だけを該当ルールで確認する。依頼がコードレビューだけなら`REVIEW_STATE.md`更新以外は変更しない。一般論より`PLAN.md`・設計・開発フェーズを優先する。
...
修正後は指摘箇所と、その修正で変わった影響範囲だけを再確認する。確認済み範囲でも対象コード・仕様・依存関係・判断前提が変わった場合は再確認し、Public API・データ構造、破壊的、セキュリティ、広範囲・高リスク変更では効率化を理由に必要な横断確認を省かない。
