...
- `GITHUB.md`のPRレビュー対応を分類・修正と返信の2条へ分割。「必要分だけ修正し、必須で返信する」が修正した分だけ返信すればよいとも読める曖昧さがあったため、分類した全コメントへ個別返信し、見送り・対応不要・質問も理由を添えて無返信で残さないことを明示した

## 2026-09-07

- 最新モデル向けに共通常時ルールとWORKFLOWを簡素化。毎回の担当AI・モデル・思考深度選択を廃止し、現在モデルでの自律実行、承認の再利用、影響・リスクに応じた検証、明確な利点がある場合だけのSub Agent・独立レビューへ統一
- 選択・モデル強制Hookを無動作互換ファイルへ縮小し、配備時に旧AIRules登録だけを解除。ユーザーHookとCodex設定を保持し、固定モデル指定・Claude Skill導線・説明・配備テストを整合

## 2026-09-02

- Sub Agentの全履歴継承、成果の返らない同条件再試行、巨大な検索・ビルド・diff出力、確認済み資料の再読による過剰なトークン消費を防ぐ規則を`WORKFLOW.md`へ追加。作業パケット化と未確認範囲の分割を既定とし、一次情報・影響範囲・高リスク変更の検証は省かない
- `REVIEW.md`へ、修正後は指摘箇所と変化した影響範囲だけを再確認し、仕様・依存関係・判断前提が変わった場合と高リスク変更は必要な横断確認を戻す精度維持条件を追加

## 2026-08-30

- 設計書を抽象的な方針だけで終えず、主要クラス・コンポーネントの責務・状態・公開操作・依存関係と、主要処理の呼出順序・状態変化・分岐・失敗時の扱いを実装者が一意に判断できる粒度で記述する規則を`DESIGN.md`へ追加。図は必須としない

## 2026-08-15

- Git管理下の実変更はworktree・PRレビューフローを既定とし、「直コミットで」「PR不要」等をユーザーが現在の作業へ明示した場合だけ省略できるよう`GIT.md`を変更

## 2026-07-29

- `REVIEW.md`の自動適用をコード変更のセルフレビュー・コードレビュー・コード変更の完了報告前だけに限定。文書編集・調査・説明・設計・企画等は自動レビューせず、非コード成果物はユーザーが明示的にレビューを依頼した場合だけ確認する

## 2026-07-28

- Codexの現在モデル・思考深度を初回に取得できない場合、即座に取得不能とせず表示直前に実行環境のセッション情報から1回取り直し、再取得にも失敗した項目だけ取得不能とする規則を追加
- Git運用へ、一塊の機能変更・追加作業が完了するごとに変更を溜めずこまめにcommitする規則を追加。共通実装規則へ、アクセス修飾子を使える言語のprivateメンバー変数・メソッドには`private`を明記する規則を追加
- Codexの現在モデル・思考深度は各実行環境がセッションへ明示した実行中値だけから取得するよう規則化。`config.toml`、親の申告、Worker起動時の指定値、過去会話による推測を禁止し、取得不能時は既定値で補わず「取得不能」と表示する

## 2026-07-27

- `WORKFLOW.md`を規範と例外を維持したまま27%圧縮。作業開始ゲートをコードの実変更だけに限定し、文書編集を除外した。Hookにも文書単独変更の除外と文書＋コード変更の停止を追加し、全53ケース成功
- CodexのWorkflow選択を、推奨を1番にした2〜3個の番号付き候補へ統一。各候補に担当・モデル・思考深度・Worker使用有無と品質・費用・時間差を含め、ユーザーは「推奨」または`1`〜`3`だけで回答できるようにした。番号なしの旧形式をStopで差し戻し、旧形式のpendingを再利用しないschema 3へ更新した
- 親Codexと選択モデルまたは思考深度が異なる場合のWorker起動を「原則」から必須へ変更。指定モデル・思考深度のWorkerへ実装を任せ、親が切り替わった扱いで直接実装することを禁止し、指定値を利用不能なら実装前に確認する
...
- モデル確認を省略できる軽微実装を、対象が既存実装1ファイル＋直接テスト1つ以内、単一箇所の誤字・文言・コメント・import・定数・比較演算子・条件式・Nullチェック、禁止境界なしを依頼文から確認できる場合に限定。「軽微」の自己申告、Public API・設定、新規ファイル、実装2ファイルは拒否。上記の状態遷移・自然な承認・番号選択・非実装除外・Node REPL判定を含む51ケースを一時HOMEで検証し全件成功
